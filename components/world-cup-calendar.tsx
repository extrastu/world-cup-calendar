      ...
      <div className='relative min-w-[300px] rounded-xl bg-card border border-border/60 p-5 hover:border-accent/40 hover:bg-card/80 transition-all duration-200'>
        ...
        <div className='flex items-center justify-between mt-5 mb-4'>
          ...
          <div className='flex flex-col items-center gap-2 w-24'>
            ...
            <span className='text-sm font-medium text-foreground/90 text-center leading-tight'>{match.homeTeam}</span>
          </div>

          {/* Time */}
          <div className='text-center px-4'>
            <div className='text-3xl font-semibold text-foreground tracking-tight'>{convertedTime.time}</div>
          ...
          <div className='flex flex-col items-center gap-2 w-24'>
            ...
            <span className='text-sm font-medium text-foreground/90 text-center leading-tight'>{match.awayTeam}</span>
          </div>
        </div>
    ...
    <div className='min-h-screen bg-background flex'>
      ...
      <aside className='hidden lg:flex flex-col w-60 border-r border-border/40 bg-sidebar sticky top-0 h-screen overflow-y-auto'>
        ...
        <div className='p-3 mx-3 mb-3 rounded-xl bg-muted/30 border border-border/40'>
          ...
          <h3 className='font-medium text-foreground text-sm mb-1'>不错过任何比赛</h3>
          <p className='text-xs text-muted-foreground mb-3 leading-relaxed'>订阅日历，开赛前收到提醒</p>
          <Link
            href='/subscribe'
        ...
        <div className='px-5 py-4 border-t border-border/40'>
          <p className='text-[11px] text-muted-foreground'>&copy; 2026 CupCalendar</p>
        </div>
      </aside>
      ...
      <div className='flex-1 flex flex-col min-w-0'>
        ...
        <header className='lg:hidden sticky top-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border/50'>
          <div className='flex items-center justify-between px-4 py-3'>
            ...
            <div className='flex items-center gap-2'>
              <Link href='/data' className='p-2 rounded-lg hover:bg-muted/50 transition-colors'>
                <Database className='w-5 h-5 text-muted-foreground' />
              </Link>
              <Link href='/subscribe' className='p-2 rounded-lg bg-accent text-accent-foreground'>
        ...
        <section className='relative overflow-hidden'>
          ...
          <div className='relative px-6 md:px-10 py-10 md:py-16'>
            ...
            <div className='flex items-center justify-between mb-10'>
              <div className='relative'>
                ...
                >
                  ...
                  <span className='text-sm text-foreground/90'>{selectedTimezone.label}</span>
                  <ChevronRight
                    className={`w-4 h-4 text-muted-foreground transition-transform duration-200 ${showTimezoneDropdown ? "rotate-90" : ""}`}
                  />
                </button>
            ...
            <div className='max-w-2xl'>
              ...
              {/* Countdown */}
              <div>
                <p className='text-[11px] text-muted-foreground uppercase tracking-widest mb-4'>距离世界杯开幕</p>
        ...
        <section className='px-6 md:px-10 py-8'>
          <div className='flex items-center justify-between mb-5'>
            <div className='flex items-center gap-3'>
              ...
              <h2 className='text-lg font-semibold text-foreground tracking-tight'>即将开始</h2>
            </div>
            <Link
              href='/matches'
          ...
          <p className='text-sm text-muted-foreground mb-5'>{selectedTimezone.label}</p>
        ...
        <section className='px-6 md:px-10 py-8 flex-1'>
          <div className='flex items-center justify-between mb-5'>
            <div className='flex items-center gap-3'>
              <div className='w-1 h-5 rounded-full bg-accent' />
              <h2 className='text-lg font-semibold text-foreground tracking-tight'>全部比赛</h2>
            </div>
          ...
          <div className='bg-card rounded-xl border border-border/40 overflow-hidden'>
            ...
            <div className='divide-y divide-border/30'>
              {filteredMatches.slice(0, 10).map((match) => {
                ...
                const awayFlag = getFlagUrl(match.awayTeam);
                const isTBD =
                  match.homeTeam.includes("W ") || match.homeTeam.includes("L ") || match.homeTeam.match(/^\d/);
                  ...
                  >
                    ...
                    <div className='col-span-5 flex items-center gap-3'>
                      <div className='flex items-center gap-2.5'>
                        ...
                        <span className='font-medium text-foreground/90'>{match.homeTeam}</span>
                      </div>
                      ...
                      <div className='flex items-center gap-2.5'>
                        ...
                        <span className='font-medium text-foreground/90'>{match.awayTeam}</span>
                      </div>
                    </div>

                    {/* Group */}
                    <div className='col-span-1 text-sm text-muted-foreground'>{match.group || "-"}</div>

                    {/* Venue */}
                    <div className='col-span-2 text-sm text-muted-foreground truncate'>{match.venue}</div>
